...
public class FlywayUtil {
    ...
    public static void migrateShardingDataSource(DynamicRoutingDataSource dynamicRoutingDataSource) {
        YamlMultientyProperties properties = SpringUtil.getBean(YamlMultientyProperties.class);
        FlywayExtensionProperties flywayExtProperties = SpringUtil.getBean(FlywayExtensionProperties.class);
        migrateShardingDataSource(dynamicRoutingDataSource.getDataSource(MultientyConstants.DS_SHARDING),
        ...
        try {
            ...
            if(m.find()) {
                ...
                if(shardingDBName.startsWith(properties.getStandaloneTenantDbNamePrefix())) {
                    // 独立站
                    location = flywayExtProperties.getLocationPrefix() + MultientyConstants.SHARDING_FLYWAY_SQL_DIR +
