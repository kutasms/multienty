...
import com.chia.multienty.core.strategy.pay.domain.partner.MTPartnerPayTransaction;
...
import com.chia.multienty.core.strategy.pay.weixin.service.WxV3PartnerPayService;
...
@Service
@Slf4j
public class WxV3PartnerPayServiceImpl implements WxV3PartnerPayService {
    ...
    public String getType() {
        return PayType.WECHAT_V3_PARTNER.name();
    }
    ...
    public MTPayRefund refund(MTRefundRequest req)  {
        ...
        request.setSubMchid(app.getSubMchId());
        request.setNotifyUrl(
                String.format(
                        properties.getWechat().getPay().getV3PartnerNotifyUrls().get("order-refund-notify-url"),
                        app.getTenantId(),
                        app.getProgramId()
                ));
    ...
    public MTPrepayResponse prepay(MTPrepayRequest req) throws Exception {
        ...
        try{
            ...
            switch (req.getSource()) {
                case MINI_PROGRAM:
                    attach = String.format(attachTemplate, TerminalType.MP_WECHAT.getValue() ,
            ...
            request.setSpAppid(properties.getWechat().getThirdParty().getAppId());
            request.setSubAppid(app.getMiniAppId());
            request.setSubMchid(app.getSubMchId());
            request.setAttach(attach);
            request.setDescription(String.format("订单%s支付", req.getTradeNo()));
            request.setSubMchid(app.getPay().getMchId());
            request.setNotifyUrl(String.format(
                    properties.getWechat().getPay().getV3PartnerNotifyUrls().get("trade-pay-notify-url"),
                    app.getTenantId(),
                    app.getProgramId()
            ));
            ...
            PrepayWithRequestPaymentResponse rsp = jsapiService.prepayWithRequestPayment(request,
                    properties.getWechat().getOa().getAppId());
    ...
    @Override
    public MTPartnerPayTransaction queryOrder(Long programId, String outTradeNo) throws Exception {
        WechatAppDTO app = dubboWechatService.getWechatApp(new WechatAppDetailGetParameter()
                .setContainsPay(true)
        ...
        request.setSpMchid(properties.getWechat().getPay().getSpMchId());
        request.setSubMchid(app.getSubMchId());
        request.setOutTradeNo(outTradeNo);

        Transaction transaction = jsapiService.queryOrderByOutTradeNo(request);
        MTPartnerPayTransaction mpt = new MTPartnerPayTransaction();
